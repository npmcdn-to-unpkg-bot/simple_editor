...
var RulersGuides = function (evt, dragdrop) {
    ...
    var doc         = document.documentElement,
        ...
        Ruler       = function (type, size) {
            ...
            var _offset = $('#mainEditorDiv>.wrap-layers').offset();
            var panelLeft = parseInt($('aside.layers-pannel').css('left'));
            if(panelLeft==0){//좌측 패널 보임
                var _left = -Math.abs(parseInt(_offset.left) - parseInt($('aside.layers-pannel').css('width')));
            }else{//좌측 패널 숨김
                var _left = -Math.abs(parseInt(_offset.left));
            }
            _left+=2;

            for (var i= (type=='h')?_left:0; i < cnt; i = i + 1) {
            ...
            $(ruler).on('mousedown',function(e){
                ...
                var horizonTop = (e.offsetY + scrollPos[0]) + 'px';
                var verticalLeft = (e.offsetX + scrollPos[0]) + 'px';

                if(e.target.nodeName=='SPAN'){
                    horizonTop = (e.offsetY + scrollPos[0]) + 'px';
                    verticalLeft - (e.clientX - targetOffset.top + scrollPos[1]) + 'px';
                }
                //div.ruler.h.unselectable error
                if (type == 'h') {
                    guide.className = 'guide h draggable';
                    //guide.style.top = (e.clientY + scrollPos[0]) + 'px';
                    //guide.style.top = (e.offsetY + scrollPos[0]) + 'px';
                    guide.style.top = horizonTop;
                    ...
                    //guide.style.left = (e.offsetX + scrollPos[1]) + 'px';
                    guide.style.left = verticalLeft;
                ...
                dragdrop.set(guide, {
                    ...
                    onstart: function (elem) {
                        ...
                        var panelLeft = parseInt($('aside.layers-pannel').css('left'));
                        if(panelLeft==0){//좌측 패널 보임
                            this._left = -Math.abs(parseInt(_offset.left) - parseInt($('aside.layers-pannel').css('width')));
                        }else{//좌측 패널 숨김
        ...
        prepare     = function () {
            ...
            setTimeout(function () {
                ...
                document.getElementById('mainEditorDiv').appendChild(wrapper);

                //var _div = document.getElementById('mainEditorDiv').children;
                //_div[0].appendChild(wrapper);
